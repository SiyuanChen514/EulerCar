...
class GrabNode(Node):
    ...
    def __init__(self):
        ...
        self.DetectBox_timer = self.create_timer(
            2.0, self.detectCallback, callback_group=rclpy.callback_groups.ReentrantCallbackGroup())
        
        self.grabTimer = self.create_timer(
            0.5, self.grabCallback, callback_group=rclpy.callback_groups.ReentrantCallbackGroup())
    ...
    def grabCallback(self):
        if self.beginGrab and self.box_location != [-1,-1]:
            if abs(self.box_location[0]) <= 0.01 and abs(self.box_location[1]) < 0.1:
                print("-----------------------Ready to Grab---------------------")
                # p为位置，q为姿态
                p = [0,0.11,0]
                q = [0,0,0,1]
            ...
            elif abs(self.box_location[0]) <= 0.01 and abs(self.box_location[1]) >= 0.1:
                print("-----------------------Too Far---------------------")
                # p为位置，q为姿态
                p = [0,self.box_location[1]-0.1,0]
                q = [0,0,0,1]
                self.box_location = [-1,-1]
                self.Navigate2(p,q)
